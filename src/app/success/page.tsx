...
export async function generateMetadata({
  ...
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_APP_URL}/api/checkout/session?session_id=${sessionId}`
  )
  if (!response.ok) {
    return {
      title: "Purchase Not Found",
...
export default async function SuccessPage({
  ...
  return (
    <main className="mt-[-64px] flex flex-col items-center justify-center h-[656px] w-full max-w-[590px] m-auto lg:mt-0">
      <h3 className="text-gray-100 text-2xl font-bold mb-16 md:text-[32px]">
        Purchase successful!
      </h3>
      ...
      <p className="text-gray-200 font-normal text-[16px] mb-10 px-6 text-center md:mb-20 md:text-2xl">
        Congratulations{" "}
        <strong className="text-gray-200 font-bold text-[16px] md:text-2xl">
        ...
        is already on the way to your house.
      </p>

      <Link href={"/"} className="text-green-300 text-[16px] font-bold md:text-xl">
        Return to catalog
      </Link>
    </main>
  )
